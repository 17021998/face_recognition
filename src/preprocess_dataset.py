...
def _process_image(filename, crop_dim):
    ...
    if image is not None:
        ...
        print('align success {}'.format(filename))
    else:
...
def _align_image(image, crop_dim):
    ...
    numface = len(detected_faces)

    if numface != 0:
        faces = dlib.full_object_detections()
        for detection in detected_faces:
            faces.append(sp(image, detection))
        aligned = dlib.get_face_chip(image, faces[0], size=crop_dim, padding=0.25)
...
"""
run:
python preprocess_dataset.py --input-dir /data/ --output-dir /data/outpub --crop-dim 180
"""